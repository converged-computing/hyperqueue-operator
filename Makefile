...
# flux-framework.org/hyperqueue-operator-bundle:$VERSION and flux-framework.org/hyperqueue-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= flux-framework.org/hyperqueue-operator
...
build-config: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default > examples/dist/hyperqueue-operator.yaml
...
test-deploy: manifests kustomize
	...
	docker push ${DEVIMG}
	cd config/manager && $(KUSTOMIZE) edit set image controller=${DEVIMG}
	$(KUSTOMIZE) build config/default > examples/dist/hyperqueue-operator-dev.yaml
...
.PHONY: clean
clean:
	kubectl delete -n hyperqueue-operator svc --all --grace-period=0 --force
	kubectl delete -n hyperqueue-operator cm --all --grace-period=0 --force
	kubectl delete -n hyperqueue-operator pods --all --grace-period=0 --force
	kubectl delete -n hyperqueue-operator jobs --all --grace-period=0 --force
	kubectl delete -n hyperqueue-operator jobset --all --grace-period=0 --force
	kubectl delete -n hyperqueue-operator hyperqueue --all --grace-period=0 --force
